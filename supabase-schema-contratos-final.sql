-- =====================================================
-- ESQUEMA PARA TABLAS DE CONTRATOS
-- Compatible con tabla 'usuario' existente
-- Primary Key: usuario_id (int2)
-- =====================================================

-- =====================================================
-- CREAR TABLAS DE CONTRATOS
-- =====================================================
...
CREATE TABLE IF NOT EXISTS contrato_determinado (
    ...
    updated_by INTEGER REFERENCES usuario(usuario_id)
);
...
CREATE POLICY "Usuarios pueden ver contratos de su nivel" ON contrato_determinado
    FOR SELECT USING (
        EXISTS (
            ...
            WHERE usuario.usuario_id = (current_setting('app.user_id', true))::integer
...
SELECT 
    cd.*,
    u.usuario_nombre || ' ' || COALESCE(u.usuario_app, '') || ' ' || COALESCE(u.usuario_apm, '') AS creado_por
FROM contrato_determinado cd
LEFT JOIN usuario u ON cd.created_by = u.usuario_id;
...
LEFT JOIN usuario u ON ci.created_by = u.usuario_id;
...
LEFT JOIN usuario u ON ch.created_by = u.usuario_id;
...
DO $$
BEGIN
    RAISE NOTICE '✅ Tablas de contratos creadas exitosamente!';
    RAISE NOTICE '✅ Tabla usuario existente: NO MODIFICADA';
    RAISE NOTICE '✅ Compatible con usuario_id (int2)';
    RAISE NOTICE '✅ Tablas: contrato_determinado, contrato_indeterminado, contrato_hora';
    RAISE NOTICE '✅ Índices, triggers, RLS y vistas configurados';
    RAISE NOTICE '⚠️  NOTA: RLS usa current_setting para identificar usuario';
